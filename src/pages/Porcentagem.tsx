  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <section className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><h2 className="text-lg font-black text-slate-900">Monte seu exemplo</h2><label className="mt-5 block text-sm font-bold text-slate-700">Valor base<input type="number" min="0" value={value} onChange={(event) => setValue(Number(event.target.value))} className="mt-2 min-h-12 w-full rounded-xl border border-slate-300 px-3 text-lg font-bold outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100" /></label><label className="mt-5 block text-sm font-bold text-slate-700">Porcentagem: <span className="text-blue-700">{formatPercentage(percentage)}</span><input type="range" min="0" max="100" step="1" value={percentage} onChange={(event) => setPercentage(Number(event.target.value))} className="mt-3 w-full accent-blue-600" /><input type="number" min="0" max="100" value={percentage} onChange={(event) => setPercentage(Math.min(100, Math.max(0, Number(event.target.value))))} className="mt-2 min-h-11 w-full rounded-xl border border-slate-300 px-3 font-bold outline-none focus:border-blue-500" /></label><button type="button" onClick={reset} className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-slate-800"><RotateCcw size={15} /> Restaurar exemplo</button></div>
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><h2 className="text-lg font-black text-slate-900">Visualização de 100 partes</h2><div className="mt-4 grid grid-cols-10 gap-1.5">{Array.from({ length: 100 }, (_, index) => <div key={index} className={`aspect-square rounded-sm ${index < filledBlocks ? 'bg-blue-600' : 'bg-slate-100'}`} aria-hidden="true" />)}</div><div className="mt-5 rounded-2xl bg-slate-50 p-4"><p className="text-sm font-semibold text-slate-600">{formatPercentage(percentage)} de {value}</p><p className="mt-1 text-3xl font-black text-blue-700">{result.amount.toLocaleString('pt-BR', { maximumFractionDigits: 2 })}</p>{mode !== 'percentage' && <p className="mt-2 text-sm text-slate-600">{mode === 'discount' ? 'Valor final após desconto:' : 'Novo valor após acréscimo:'} <strong className="text-slate-900">{result.total.toLocaleString('pt-BR', { maximumFractionDigits: 2 })}</strong></p>}</div>{mode === 'percentage' ? <p className="mt-4 text-sm text-slate-600">A porcentagem indica quantas partes de cada 100 estão sendo consideradas.</p> : <p className="mt-4 flex items-center gap-2 text-sm font-semibold text-slate-600">{mode === 'discount' ? <Minus size={16} /> : <Plus size={16} />} O cálculo mostra o impacto percentual sobre o valor base.</p>}</div>
      </section>
